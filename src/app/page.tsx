...
function InlineFaqCard({ question, answer }: { question: string; answer: string }) {
  ...
  return (
    <div 
      // UPDATED: Glassmorphism + Glowing Hover/Active Shadows for Mobile
      className="glass card-premium p-6 rounded-2xl cursor-pointer group hover:-translate-y-2 active:-translate-y-1 hover:shadow-[0_0_35px_rgba(244,114,182,0.4)] active:shadow-[0_0_35px_rgba(244,114,182,0.5)] transition-all duration-500 relative overflow-hidden bg-white/30 backdrop-blur-xl border border-white/60"
      onClick={() => setIsOpen(!isOpen)}
    >
      {/* Background Hover Gradient Reveal */}
      <div className="absolute inset-0 bg-theme-gradient opacity-0 group-hover:opacity-10 group-active:opacity-10 transition-opacity duration-500 -z-10"></div>
      
      <div className="flex justify-between items-center relative z-10">
        {/* Text turns to gradient on hover and active (touch) */}
        <h3 className="text-lg font-bold text-gray-900 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-blue-600 group-hover:via-purple-500 group-hover:to-pink-500 group-active:text-transparent group-active:bg-clip-text group-active:bg-gradient-to-r group-active:from-blue-600 group-active:via-purple-500 group-active:to-pink-500 pr-4 transition-all duration-300">
          {question}
        </h3>
        <motion.div animate={{ rotate: isOpen ? 180 : 0 }} transition={{ duration: 0.3, ease: "easeInOut" }}>
          <ChevronDown className="text-pink-500 w-6 h-6 flex-shrink-0" />
        </motion.div>
      </div>
          ...
          {steps.map((step, index) => (
            <div key={index} className="relative w-[100vw] h-full flex-shrink-0 flex items-center justify-center pointer-events-none">
              <div className={`absolute ${step.pos} w-[85vw] sm:w-[400px] pointer-events-auto`}>
                <div className="glass card-premium p-8 rounded-[2.5rem] bg-white/40 backdrop-blur-2xl shadow-xl group hover:-translate-y-4 active:-translate-y-2 hover:shadow-[0_0_40px_rgba(244,114,182,0.5)] active:shadow-[0_0_40px_rgba(244,114,182,0.6)] transition-all duration-500 overflow-hidden relative">
                  ...
                  <div className="w-14 h-14 rounded-full bg-theme-gradient text-white group-hover:bg-white group-active:bg-white group-hover:text-pink-500 group-active:text-pink-500 flex items-center justify-center font-black text-2xl mb-6 shadow-lg transition-colors duration-500">
                    {step.num}
                  </div>
      ...
      <section id="about" className="py-24 relative px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <ScrollReveal direction="right" className="order-2 md:order-1">
            <div className="glass card-premium p-8 md:p-12 rounded-3xl relative hover:-translate-y-3 active:-translate-y-2 hover:shadow-[0_0_40px_rgba(96,165,250,0.2)] active:shadow-[0_0_40px_rgba(96,165,250,0.3)] transition-all duration-500">
              ...
              <motion.button 
                ...
                className="relative overflow-hidden px-10 py-4 rounded-full font-bold bg-white border border-white/80 group shrink-0 shadow-[0_4px_15px_rgba(0,0,0,0.1)] hover:shadow-[0_0_30px_rgba(96,165,250,0.7)] active:shadow-[0_0_30px_rgba(96,165,250,0.8)] hover:-translate-y-1 active:-translate-y-1 transition-all duration-500"
              >
                <motion.div 
                  variants={{
                    initial: { height: "0%" },
          ...
          <ScrollReveal direction="left" className="order-1 md:order-2 h-96 w-full rounded-3xl glass card-premium flex items-center justify-center overflow-hidden relative group hover:-translate-y-3 active:-translate-y-2 hover:shadow-[0_0_40px_rgba(244,114,182,0.2)] active:shadow-[0_0_40px_rgba(244,114,182,0.3)] transition-all duration-500">
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-br from-blue-100/30 to-pink-100/30 group-hover:opacity-0 group-active:opacity-0 transition-opacity duration-500"></div>
          </ScrollReveal>
        </div>
      </section>
              ...
              <div className="glass card-premium p-6 rounded-2xl hover:-translate-y-2 active:-translate-y-1 hover:shadow-[0_0_20px_rgba(96,165,250,0.2)] active:shadow-[0_0_20px_rgba(96,165,250,0.3)] transition-all duration-500">
                ...
                <form className="flex flex-col sm:flex-row gap-4" onSubmit={(e) => e.preventDefault()}>
                  ...
                  <motion.button 
                    ...
                    whileInView="animate"
                    viewport={{ once: true, margin: "-100px" }}
                    className="relative overflow-hidden px-10 py-4 rounded-full font-bold bg-white border border-white/80 group shrink-0 shadow-md hover:shadow-[0_0_30px_rgba(244,114,182,0.8)] active:shadow-[0_0_30px_rgba(244,114,182,0.9)] hover:-translate-y-1 active:-translate-y-1 transition-all duration-500"
                  >
                    <motion.div 
                      variants={{
                        initial: { height: "0%" },
